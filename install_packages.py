...
platform_check = bool(platform.system() == "Windows" and platform.version().startswith("10")) or (platform.system() != "Windows")
...
Optional dependencies:
 - PyInstrument""")
	while True:
		choice = (input("Please select which package you'd like to install (all/required/optional): ")).lower()
	...
	while True:
		inst = (input("Would you like to install with conda or pip? (default: pip): ")).lower()
		if not inst:
			inst = "pip"
	...
	if inst == "pip":
		if choice in {"optional", "all"}:
			subprocess.check_call([sys.executable, "-m", "pip", "install", "pyinstrument"])
			if choice == "optional":
				raise SystemExit
		subprocess.check_call([sys.executable, "-m", "pip", "install", "colorama==0.4.4"])
